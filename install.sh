...
# The line that says a generated file is ours to rewrite. Carried by every unit
# and plist this installer writes from now on, so a later version can recognise
# its own work without keeping a table of everything it has ever written. Two
# spellings because the two formats spell a comment differently, and a plist
# cannot say anything before its XML declaration.
GENERATED_MARKER="# Generated by PocketTUI install.sh — rewritten on update; edit a copy under another name to customise."
GENERATED_MARKER_XML="<!-- Generated by PocketTUI install.sh -->"
...
Restart=always
RestartSec=10
# The tmux server holding the user's sessions is spawned by app.py, so it lands
# in this unit's cgroup; the default KillMode=control-group would kill it — and
# every session with it — on every restart, including the one an update does.
# Only app.py itself should be stopped.
KillMode=process
...
# deliberately a second copy and must NOT be kept in step with UNIT_CONTENT
# above: it does not describe what we write, it describes the file already on
# disk at every install made before this version, so that such a file can be
# recognised as ours exactly once — after which it carries the marker and this
# is never consulted again. Editing it to match a future template would only
# make it stop matching the thing it exists to identify.
UNIT_CONTENT_LEGACY="$(cat <<EOF
[Unit]
Description=PocketTUI tmux terminal backend (port $PORT)
After=network.target
StartLimitIntervalSec=0
...
# The plist as it shipped before the marker — frozen for the same reason and
# under the same rule as UNIT_CONTENT_LEGACY: it identifies what is on disk at
# existing installs, not what we write. Nothing about the agent itself changed,
# so this is only here to keep a macOS install from being stuck on its first
# plist the way the systemd side was.
AGENT_CONTENT_LEGACY="${AGENT_CONTENT/$'\n'"$GENERATED_MARKER_XML"/}"
...
if [[ "$HAVE_SYSTEMD" == "1" ]]; then
    ...
    elif [[ "$UNIT_EXISTS" == "1" ]] && [[ "$UNIT_OURS" == "1" ]]; then
        # An older version of our own unit. Replacing it is the only way a fix
        # to the unit itself ever reaches an install that already exists — the
        # branch below would leave every one of them on the shape they were
        # first installed with, forever. No backup: this is not the user's file,
        # and the copy being replaced is one this installer wrote and can write
        # again. A unit someone has customised is not this case; it fails both
        # tests in generated_by_us and lands below.
        vsay "  $UNIT_PATH is one this installer wrote — rewriting it."
        if ! printf '%s\n' "$UNIT_CONTENT" > "$UNIT_PATH" 2>/dev/null; then
            say "  WARNING: could not rewrite $UNIT_PATH — starting what is there."
            note "could not rewrite $UNIT_PATH (kept the old one)"
            start_service "kept the existing $UNIT_PATH"
        else
            start_service "updated $UNIT_PATH"
        fi
    ...
    elif [[ "$AGENT_EXISTS" == "1" ]] && [[ "$AGENT_OURS" == "1" ]]; then
        # Our own earlier plist — the systemd branch's reasoning, in launchd's
        # spelling. start_agent unloads before it bootstraps, so a rewritten
        # plist is picked up rather than ignored.
        vsay "  $AGENT_PATH is one this installer wrote — rewriting it."
        if ! printf '%s\n' "$AGENT_CONTENT" > "$AGENT_PATH" 2>/dev/null; then
            say "  WARNING: could not rewrite $AGENT_PATH — loading what is there."
            note "could not rewrite $AGENT_PATH (kept the old one)"
            start_agent "kept the existing $AGENT_PATH"
        else
            start_agent "updated $AGENT_PATH"
        fi
