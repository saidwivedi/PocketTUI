...
@media (prefers-color-scheme: dark) {
  :root { --bg: #1c1a17; --fg: #e8e2d6; --dim: #97907f; --line: #4a443b; }
}
...
# Failures a scheme could be the whole of. A target addressed as https that
# never completes a handshake is as often an intranet box that only speaks
# http, and the other way round; the rest — a 502 from the target, a loop, an
# expired token — say nothing about the scheme and get no such link.
BROWSE_ALT_CODES = ("tls", "refused", "timeout", "unknown_host")
...
button {{ font: inherit; padding: .5em 1.4em; border-radius: 8px;
  ...
  cursor: pointer; }}
a.alt {{ display: inline-block; margin-left: 1em; color: inherit; }}
</style></head>
<body><div class="card">
<h1>{title}</h1>
<p>{detail}</p>
<button onclick="location.reload()">Retry</button>{alt}
</div>
<script>
...
def browse_error_page(code: str, detail: str, origin: str | None,
    ...
    link = ""
    # The scheme is read back off the path the helper built rather than passed
    # beside it: the letter sits in a fixed segment of this proxy's own shape,
    # so there is one place it can be, and the label cannot drift from the link.
    m = re.search(r"/b/[^/]+/([hs])/", alt or "") if code in BROWSE_ALT_CODES \
        else None
    if m:
        link = (f'<a class="alt" href="{html_escape(alt)}">'
                f"Try {'http' if m.group(1) == 'h' else 'https'}:// instead</a>")
...
def browse_other_scheme(rec: "BrowseToken", sch: str, hostport: str, path: str,
                        query: str) -> str | None:
    """The same target addressed the other way, or None if there is no other.

    Only where the port is the scheme's own default, because that is the case
    where no port was ever meant: `box.example.net` typed with no scheme is
    filled in as https and 443, and an intranet box that only speaks http is
    then a handshake that never completes. A port somebody wrote out names one
    server, and offering it under the other scheme would be a guess about a
    thing that was not being guessed at.
    """
    host, sep, port = hostport.rpartition(":")
    if not sep or not host:
        return None
    if sch == "s" and port == "443":
        other, other_port = "h", "80"
    elif sch == "h" and port == "80":
        other, other_port = "s", "443"
    else:
        return None
    dest = f"{rec.prefix}/b/{rec.token}/{other}/{host}:{other_port}{path}"
    return f"{dest}?{query}" if query else dest


def browse_error_response(code: str, detail: str, origin: "str | None",
                          sandbox: bool = True,
                          alt: "str | None" = None) -> Response:
    """One failure as the page the iframe renders and the pane listens to."""
    return Response(browse_error_page(code, detail, origin, alt),
